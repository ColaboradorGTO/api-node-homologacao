    ...
    if(e.checked) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          const produtosNaPagina = dados.map(item => item.IDPRODUTO);
          setSelectAll(produtosNaPagina.length === selectedIds.length);
          const updatedSelectedIds = [...new Set([...selectedIds, ...produtosNaPagina])];
          setSelectedIds(updatedSelectedIds);
          const novosProdutosSelecionados = dados.filter(item => !produtosSelecionados.some(prod => prod.IDPRODUTO === item.IDPRODUTO));
          setProdutosSelecionados([...produtosSelecionados, ...novosProdutosSelecionados]);
          setBtnVisivel(true);
        }
      });
    } else {
